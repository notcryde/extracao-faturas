...
def home():
    ...
    if faturas:
                    ...
                    with st.spinner('O sistema está processando as faturas...'):
                        try:
                            ...
                            if processed_dataframe.empty:
                                ...
                                mes_comp_limpo = mes_competencia.replace('/', '_')
                                st.download_button(
                                    label='Baixar Planilha',
                                    data=csv_file_bytes,
                                    file_name=f'Faturas_{concessionaria}_{mes_comp_limpo}.csv',
                                    mime='text/csv'
                                )
                    ...
                    with st.spinner('O sistema está processando o relatório e agrupando os arquivos...'):
                        try:
                            ...
                            if processed_dataframe.empty:
                                ...
                                if tipo_debito == 'Automático':
                                    processed_dataframe = processed_dataframe[processed_dataframe['debito'] != '']
                                    mensagem_erro = 'Não há faturas com débito automático'
                                else:
                                    processed_dataframe = processed_dataframe[processed_dataframe['debito'] == '']
                                    mensagem_erro = 'Não há faturas com débito manual'
        ...
        if option == 'Relatório' and st.session_state.get('report_data') is not None:
            mes_comp_limpo = mes_competencia.replace('/', '_')
            local_limpo = local.replace(' ', '_')
            tipo_debito_limpo = tipo_debito.replace(' ', '_')
            
            st.download_button(
                label='Baixar Relatório',
                data=st.session_state.report_data,
                file_name=f'Relatorio_{mes_comp_limpo}_{local_limpo}_{concessionaria}_Debito_{tipo_debito_limpo}.xlsx',
            ...
            if st.session_state.get('zip_data') is not None:
                st.download_button(
                    ...
                    file_name=f'ZIP_{mes_comp_limpo}_{local_limpo}_{concessionaria}_Debito_{tipo_debito_limpo}.zip',
